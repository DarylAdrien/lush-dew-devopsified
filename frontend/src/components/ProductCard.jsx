...
const ProductCard = ({ product }) => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      try {
        const response = await axios.get("http://lushdew.local/api/user/info/", {
  ...
  return (
    <div className="col-md-4">
      <div className="card h-100 text-center position-relative" onClick={goToProductPage} style={{ cursor: "pointer" }}>
        ...
        <img src={`http://lushdew.local${product.image}`} alt={product.title} className="card-img-top mx-auto" style={{ width: "80%", height: "auto" }} />
